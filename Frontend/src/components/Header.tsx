...
import { FaTachometerAlt, FaSignOutAlt, FaUser, FaClipboardList, FaCog, FaHome, FaComments } from 'react-icons/fa'
...
const Header = ({ currentView = 'home', onNavigate }: HeaderProps) => {
  ...
  const handleNavClick = (view: 'home' | 'dashboard' | 'chat') => {
    if (onNavigate) {
      onNavigate(view)
    }
        ...
        <div className="max-w-6xl mx-auto px-8 flex justify-between items-center flex-wrap">
          ...
          <nav className="flex items-center gap-6 w-full md:w-auto justify-end md:justify-center">
            <ul className="hidden md:flex items-center gap-2">
              ...
              <li>
                <a href="#lost-items" className="flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium text-gray-900 dark:text-white hover:bg-white/10 transition-all">
                  Lost Items
                </a>
              </li>
              <li>
                <a href="#found-items" className="flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium text-gray-900 dark:text-white hover:bg-white/10 transition-all">
                  Found Items
                </a>
              </li>
              <li>
                <a href="#report" className="flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium text-gray-900 dark:text-white hover:bg-white/10 transition-all">
                  Report Item
                </a>
              </li>
              <li>
                ...
                <div className="relative">
                  ...
                  >
                    <span className="w-7 h-7 rounded-full bg-primary-50 text-primary-700 flex items-center justify-center font-semibold text-sm">
                      {user.name ? user.name.split(' ').map(n => n[0]).slice(0,2).join('').toUpperCase() : <FaUser />}
                    </span>
                    <span className="hidden sm:inline text-sm text-gray-900 dark:text-white">{user.name}</span>
                  </button>
                  ...
                  {showUserMenu && (
                    <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-gray-200 py-2 z-50">
                      ...
                      >
                        <span className="text-sm"><FaSignOutAlt /></span> Logout
                      </button>
                    </div>
                  )}
                </div>
